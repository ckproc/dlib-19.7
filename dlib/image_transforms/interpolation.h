...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        // image pyramid needs to be.   get_face_chip_details
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        else
        {
			...
			*/
			const double mean_face_shape_x[] = {
				0.113095, 0.178571, 0.261905, 0.345238, 0.428571, 0.589286, 0.672619, 0.761905, 
				0.839286, 0.904762, 0.505952, 0.505952, 0.505952, 0.505952, 0.410714, 0.458333, 
				0.505952, 0.553571, 0.60119, 0.208333, 0.267857, 0.327381, 0.375, 0.321429, 0.261905, 
				0.636905, 0.690476, 0.75, 0.803571, 0.75, 0.690476, 0.339286, 0.404762, 0.464286, 0.5, 
				0.541667, 0.60119, 0.666667, 0.60119, 0.547619, 0.505952, 0.464286, 0.404762, 0.363095,
				0.464286, 0.505952, 0.547619, 0.642857, 0.547619, 0.505952, 0.464286
				
			};
			const double mean_face_shape_y[] = {
                0.326271, 0.275424, 0.266949, 0.275424, 0.305085, 0.29661, 0.271186, 0.262712, 0.275424, 
				0.326271, 0.394068, 0.457627, 0.516949, 0.584746, 0.648305, 0.652542, 0.661017, 0.652542, 
				0.648305, 0.402542, 0.377119, 0.381356, 0.423729, 0.423729, 0.419492, 0.419492, 0.385593, 
				0.381356, 0.402542, 0.419492, 0.423729, 0.792373, 0.762712, 0.745763, 0.754237, 0.745763, 
				0.762712, 0.788136, 0.813559, 0.826271, 0.830508, 0.826271, 0.813559, 0.788136, 0.783898, 
				0.783898, 0.779661, 0.783898, 0.775424, 0.779661, 0.779661,
            };
            ...
            for (unsigned long i = 17; i < det.num_parts(); ++i)
            {
                ...
                dpoint p;
				//size = det.get_rect().height;
				//det.get_rect().height
				double padding_y = padding*det.get_rect().width()/det.get_rect().height();
                p.x() = (mean_face_shape_x[i-17]-padding)*det.get_rect().width()/(1-2*padding);
                //p.x() = (mean_face_shape_x[i-17])*det.get_rect().width();
                p.y() = (mean_face_shape_y[i-17]-padding_y)*det.get_rect().height()/(1-2*padding_y);
                //p.y() = (mean_face_shape_y[i-17])*det.get_rect().height();
        ...
        //unsigned long size_c = size*det.get_rect().width()/det.get_rect().height();
		//std::cout<<size<<":"<<size_c<<std::endl;
        //return chip_details(from_points, to_points, chip_dims(size, size));
        return chip_details(from_points, to_points, chip_dims(det.get_rect().height(), det.get_rect().width()));
    }
